...
/**
  ...
  */
abstract class Transition[State](val q:State, val q1:State) extends types {
...
/**
  ...
  * @tparam State
  */
case class OrdinaryTransition[State](override val q: State, σ: Char, override val q1: State) extends Transition[State](q, q1) {

  override def &(other:Transition[State]):Option[Transition[State2]] = {

    other match {

      case OrdinaryTransition(q:State, σ:Char, q1:State) => {

        if(this.σ == σ) Some(OrdinaryTransition[State2]((this.q, q), this.σ, (this.q1, q1)))
        else None
      }

      case RangeTransition(q:State, σ:Range, q1:State) => {

        if(this.σ >= σ.min  && this.σ <= σ.max)Some(OrdinaryTransition[State2]((this.q, q), this.σ, (this.q1, q1)))
        else None

      }

      case OperationsTransition(q, s, v, q1) => None
    }
  ...
  override def equals(that: Any): Boolean = {
    ...
    that match {
      ...
      case that: OrdinaryTransition[State] =>  {
        ...
        that.canEqual(this) && that.q == this.q && that.q1 == this.q1 && that.σ == this.σ && this.hashCode == that.hashCode
      }
      case _ => false
    }
  }
...
/**
  * Represents a transition labeled with a range of characters.
  * @param q
  * @param σ
  * @param q1
  * @tparam State
  */
case class RangeTransition[State](override val q: State, σ: Range, override val q1: State) extends Transition[State](q, q1) {
  ...
  override def &(other:Transition[State]):Option[Transition[State2]] = {
    ...
    other match {
      ...
      case OrdinaryTransition(q:State, σ:Char, q1:State) => {
        ...
        if(σ >= this.σ.min && σ <= this.σ.max) Some(OrdinaryTransition[State2]((this.q, q), σ, (this.q1, q1)))
        else None
      }
      ...
      case RangeTransition(q:State, σ:Range, q1:State) => {
        ...
        if(this.σ.min <= σ.max && σ.min <= this.σ.max) {
          ...
          if(r.min != r.max) Some(RangeTransition[State2]((this.q, q), r, (this.q1, q1)))
          else Some(OrdinaryTransition[State2]((this.q, q), r.min.toChar, (this.q1, q1)))
        }
        else None
...
/**
  * Represents a transition labeled with a set of variable operations.
  * @param q
  * @param S
  * @param q1
  * @tparam State
  */
case class OperationsTransition[State](override val q: State, S: SVOps[SVOp], V:SVars[SVar], override val q1: State) extends Transition[State](q, q1) {

  override def &(other:Transition[State]):Option[Transition[State2]] = {

    other match {

      case OrdinaryTransition(q:State, σ, q1:State) => None

      case RangeTransition(q:State, σ:Range, q1:State) => None
